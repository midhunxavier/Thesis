...
\fi}
\newcommand{\commentIB}[1]{\commentColor{IB}{#1}{teal}}
\newcommand{\IB}[1]{\textcolor{orange}{#1}}
\newcommand{\soutIB}[1]{\IB{\st{#1}}}
\newcommand{\corrIB}[2]{\soutIB{#1} \IB{#2}}
%\newcommand{\corrIB}[2]{\IB{#2}}
...
\newcommand{\commentAP}[1]{\commentColor{AP}{#1}{olive}}
\newcommand{\AP}[1]{\textcolor{olive}{#1}}
\newcommand{\soutAP}[1]{\AP{\st{#1}}}
\newcommand{\corrAP}[2]{\soutAP{#1} \AP{#2}}
...
%Formal verification, in particular, model checking allows ensuring that the model satisfies its specification by extensive check of its state space in search for violations. The drawback of this method is a state space explosion problem for even moderate industrial-sized models. To avoid this, various heuristics and bounded model checking are applied, these avail in catching most of the issues, however, room for unconsidered error remains.
%in case, i.e., if some external conditions were not taken into account. 
%Therefore, to track that the system operates as expected, online verification methods can be utilized. 
%There exist different means for implementing online verification, 
...
%	This paper addresses the challenge of offline verification of a supervised IEC~61499 model of an automation system. One of the approaches to ensure that the system satisfies its requirements is formal verification and within this paper, we focus on the model-checking approach. However, this technique requires time and effort, mainly when applied to complex systems consisting of numerous function blocks. Substituting the system with its set of abstractions for the functions to be checked reduces the time of (or, in some scenarios, makes possible) model checking. Abstraction models, in turn, may lack critical scenarios of the real systems, which, therefore will be missed during the verification process and allowed during the operation. Adding observers (or monitors) for monitoring the behavior of the system online prevents the malfunction in this case, however, the correctness of the observers themselves also should be ensured. In this paper, we present a method of formal verification of the observers for the system developed using IEC~61499 and apply our approach to a system of two pneumatic cylinders.
	%Designing a monitor for a cyber-physical system model allows us to override the step of formal verification with suitable software. The problem is that, on a complex system, monitors cannot identify a possible singularity misconception of the device with online verification which doesn't take into account all reachable states. This work is aimed at overcoming this issue to formally verify a system with its monitor. The IEC 61499 application together with an abstract plant model is then converted to the language of a model checker NuSMV for subsequent verification.
    
\end{abstract}

\begin{IEEEkeywords}
    Formal verification, model checking, monitors, IEC~61499.
\end{IEEEkeywords}
...
%\section*{ACRONYMS}
%\symboldescription{CAT}{Composite Automation Type}
%\symboldescription{ECC}{Execution Control Chart}
%\symboldescription{FB}{Function Block}
%\symboldescription{FBD}{Function Block Diagram}
%\symboldescription{LTL}{Linear Temporal Logic}
%\symboldescription{CTL}{Computation Tree Logic}
%\symboldescription{NDT}{Non Deterministic Transition}


%\section*{NOMENCLATURE}
%FB names in IEC61499 used in LTL expressions :\\
%\symbolnomenclature{$VerCTL$}{Vertical cylinder controller}
%\symbolnomenclature{$HorCTL$}{Horizontal cylinder controller}
%\symbolnomenclature{$obs$}{Monitor}
...
%\begin{figure}[h!]
%    \centering
%    \includegraphics[width=0.48\textwidth]{pic/jumeau_V1.png}
...
%\begin{figure}[h!]
%    \centering
%    \includegraphics[width=0.48\textwidth]{pic/mointorV2.png}
...
%\begin{figure}[h!]
%    \centering
%    \includegraphics[scale=0.5]{pic/HMI_collide.png}
...
%\begin{figure}[h!]
%    \centering
%    \includegraphics[width=0.45\textwidth]{IEC_syst_col.JPG}
...
%\begin{figure}[h!]
%    \centering
%    \includegraphics[width=0.3\textwidth]{systeme_col.JPG}
...
\bibliography{refrencias_sobraep}

%\newpage
%\begin{figure*}[h!]
%    \centering
%    \includegraphics[width=0.9\textwidth]{pic/vizu_cx_dys.png}
%    \caption{Counter-example visualiser of the formal verification of the monitor implemented in the dysfunctional model of the system}
%    \label{vizu_dys}
%\end{figure*}

%\begin{figure*}[h!]
%    \centering
%    \includegraphics[width=0.9\textwidth]{pic/vizu_jumeauV1.png}
%    \caption{Counter-example visualiser of the formal verification of the monitor implemented in the first version of the twin system}
%    \label{vizu_twin1}
%\end{figure*}

%\begin{figure*}[h!]
%    \centering
%    \includegraphics[width=0.9\textwidth]{pic/vizu_jumeauV2_fail.png}
%    \caption{Counter-example visualiser of the formal verification of the monitor implemented in the latest version of the twin system}
%    \label{vizu_twin2}
%\end{figure*}
